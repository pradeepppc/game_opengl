...
Monster::Monster(float x, float y , float z) {
    ...
    this->object = create3DObject(GL_TRIANGLES, 12*3, vertex_buffer_data, COLOR_ENEMY, GL_FILL);
...
void Monster::tick() {
    ...
    if(this->move == 0) //move along x-axis
    {
      if(this->speedx == 0)
      this->speedx = this->speed;
      float diff = this->position.x - this->init_x;
      if(diff <= -7)
      {
        this->speedx = this->speed;
      }
    ...
    if(this->move == 1) //move along y-axis
    {
      if(this->speedy == 0)
      this->speedy = this->speed;
      float diff = this->position.y - this->init_y;
      if(diff <= -7)
      {
        this->speedy = this->speed;
      }
    ...
    if(this->move == 2) //move along z-axis
    {
      if(this->speedz == 0)
      this->speedz = this->speed;

      float diff = this->position.z - this->init_z;
      if(diff <= -7)
      {
        this->speedz = this->speed;
      }
